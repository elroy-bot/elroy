...
from ..llm.stream_parser import ApprovalRequest, AssistantToolResult

logger = get_logger()


def exec_function_call(ctx: ElroyContext, function_call: FunctionCall) -> Generator[BaseModel, None, None]:
    function_to_call = ctx.tool_registry.get(function_call.function_name)
    if not function_to_call:
        yield AssistantToolResult(content=f"Function {function_call.function_name} not found", is_error=True)
        return

    error_msg_prefix = f"Error invoking tool {function_call.function_name}:"  # hopefully we don't need this!

    try:
        if function_call.function_name == "run_shell_command":
            yield ApprovalRequest(content=f"Should I run this command? {function_call}")
        ...
        yield AssistantToolResult(content=f"{error_msg_prefix} {e}", is_error=True)

    except Exception as e:
        yield AssistantToolResult(
