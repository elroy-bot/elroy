...
from ...utils.clock import string_to_timedelta
...
def do_create_goal(
    ...
    if existing_goal:
        ...
        goal = Goal(
            ...
            priority=priority,
            target_completion_time=ctx.clock.utc_now() + string_to_timedelta(time_to_completion) if time_to_completion else None,
        )  # type: ignore
        ctx.db.add(goal)
        ctx.db.commit()
        ctx.db.refresh(goal)
