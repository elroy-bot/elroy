...
from ...utils.clock import Clock, ensure_utc
...
def get_time_since_most_recent_user_message(clock: Clock, context_messages: Iterable[ContextMessage]) -> Optional[timedelta]:
    return pipe(
        context_messages,
        filter(lambda x: x.role == USER),
        last_or_none,
        lambda x: (clock.utc_now() - x.created_at) if x and x.created_at else None,
    )  # type: ignore
...
def compress_context_messages(
    ...
    for msg in reversed(prev_messages):  # iterate in reverse order
        ...
        if current_token_count > context_refresh_target_tokens:
            break
        elif msg_created_at is not None and msg_created_at < clock.utc_now() - max_in_context_message_age:
