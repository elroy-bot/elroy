...
def _service(ctx: ElroyContext) -> ReminderOperationService:
...
def create_onboarding_due_item(ctx: ElroyContext, preferred_name: str) -> None:
    _service(ctx).create_onboarding_due_item(preferred_name)
...
def do_create_due_item(
    ...
    return _service(ctx).do_create_due_item(name, text, trigger_time, trigger_context)


def do_complete_due_item(ctx: ElroyContext, item_name: str, closing_comment: str | None = None) -> str:
    return _service(ctx).do_complete_due_item(item_name, closing_comment)


def do_delete_due_item(ctx: ElroyContext, item_name: str, closing_comment: str | None = None) -> str:
    return _service(ctx).do_delete_due_item(item_name, closing_comment)
