...
from .transforms import Embeddable, to_memory_metadata


def upsert_embedding_if_needed(ctx: ElroyContext, row: Embeddable) -> None:
...
def add_to_context(ctx: ElroyContext, memory: Embeddable) -> None:
    ...
    if is_in_context(context_messages, memory):
        ...
        add_context_messages(
            ...
            [
                ContextMessage(
                    role=SYSTEM,
                    memory_metadata=[to_memory_metadata(memory)],
...
def add_to_current_context_by_name(ctx: ElroyContext, name: str, memory_type: Type[Embeddable]) -> str:
...
def remove_from_context(ctx: ElroyContext, memory: Embeddable):
...
def drop_from_context_by_name(ctx: ElroyContext, name: str, memory_type: Type[Embeddable]) -> str:
    item = ctx.db.exec(select(memory_type).where(memory_type.name == name)).first()  # type: ignore
