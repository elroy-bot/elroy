...
from .config import get_user_token, init_cli_io, init_config, init_elroy_context
...
def handle_message(ctx: typer.Context):
    ...
    if sys.stdin.isatty() and not ctx.params.get("message"):
        with init_elroy_context(init_cli_io(ctx), config, user_token) as (context, _new_user_created):
            pipe(
        ...
        with init_elroy_context(StdIO(), config, user_token) as (context, _new_user_created):
...
def handle_chat(ctx: typer.Context):
    if sys.stdin.isatty():
        with init_elroy_context(init_cli_io(ctx), init_config(ctx), get_user_token(ctx)) as (context, new_user_created):
            if new_user_created:
