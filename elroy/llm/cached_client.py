...
class CachedLLMClient(LLMClient):
    ...
    def _save_to_cache(self, cache_path: Path, request_data: Dict[str, Any], response: Any) -> None:
        ...
        try:
            cache_data = {"request": request_data, "response": response}
    ...
    def query_llm(self, model: ChatModel, prompt: str, system: str) -> str:
        ...
        cache_key = self._get_cache_key(model=model.name, prompt=prompt, system=system, method="query_llm")
        ...
        # Save to cache
        request_data = {"model": model.name, "prompt": prompt, "system": system, "method": "query_llm"}
    ...
    def query_llm_with_word_limit(self, model: ChatModel, prompt: str, system: str, word_limit: int) -> str:
        ...
        cache_key = self._get_cache_key(
            model=model.name, prompt=prompt, system=system, word_limit=word_limit, method="query_llm_with_word_limit"
        )
    ...
    def get_embedding(self, model: EmbeddingModel, text: str) -> List[float]:
        ...
        cache_key = self._get_cache_key(model=model.name, text=text, method="get_embedding")
        ...
        # Save to cache
        request_data = {"model": model.name, "text": text, "method": "get_embedding"}
        self._save_to_cache(cache_path, request_data, response)
    ...
    def generate_chat_completion_message(
        ...
        return super().generate_chat_completion_message(chat_model, context_messages, tool_schemas, enable_tools, force_tool)
