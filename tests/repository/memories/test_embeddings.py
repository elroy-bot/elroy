...
def test_embeddings(george_ctx):
    ...
    with open_turn_context(george_ctx) as turn:
        ...
        reminder = user_session.db.exec(
            select(AgendaItem).where(AgendaItem.user_id == user_session.user_id).order_by(desc(AgendaItem.id))
        ).first()
    ...
    assert user_session.db.get_embedding(reminder) is not None, "Embedding was not created for the reminder"
    assert user_session.db.get_embedding_text_md5(reminder) is not None, "Embedding text MD5 was not created for the reminder"
