...
def parse_args():
    ...
    # user fact expansion
    parser.add_argument(
        "--merge_key_expansion_into_value", type=str, choices=["merge", "replace", "none"], default="none"
...
def prepare_prompt(
    ...
    # get retrieved chunks
        ...
        for ret_result_entry in entry["retrieval_results"]["ranked_items"]:
            ...
            converted_turn_id = (
                int(ret_result_entry["corpus_id"].replace("noans_", "answer_").split("_")[-1]) - 1
            ...
            if merge_key_expansion_into_value is None or merge_key_expansion_into_value == "none":
                ...
                retrieved_chunks.append(
                    (corpusid2date[converted_corpus_id], corpusid2retvalue[ret_result_entry["corpus_id"]], cur_round_data)
                )
            else:
                raise NotImplementedError
        ...
        for ret_result_entry in entry["retrieval_results"]["ranked_items"]:
            ...
            if merge_key_expansion_into_value is None or merge_key_expansion_into_value == "none":
                if useronly:
                    retrieved_chunks.append(
                        (
                            corpusid2date[ret_result_entry["corpus_id"].replace("noans_", "answer_")],
        ...
        for i, cur_item in enumerate(retrieved_chunks):
            ...
            kwargs = {
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": con_prompt.format(chunk_date, json.dumps(chunk_entry), question_date_string, question_string),
                    }
                ],
                "n": 1,
                "temperature": 0,
...
def main(args):
    ...
    for entry in tqdm(in_data):
        ...
        try:
            print(
                json.dumps({"question_id": entry["question_id"], "question": entry["question"], "answer": entry["answer"]}, indent=4),
                flush=True,
            )
